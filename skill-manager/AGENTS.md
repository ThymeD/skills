...
| 手动创建 | `~/.config/opencode/skills/` | 用户自行创建 |
...
- **手动创建技能**: `~/.config/opencode/skills/<skill-name>/`
...
**首次使用或缓存文件被删除时：**
- 扫描 `~/.config/opencode/skills/` 目录（手动创建的技能）
...
function scanSkills():
  skills = []
  for dir in [~/.config/opencode/skills/, ~/.agents/skills/]:
